...
class DQN(object):
    ...
    def singleBatch(self, batch, params, weight_correction=False):
        ...
        non_final_next_states = torch.Tensor(
            [batch.next_state[i] for i in range(batch_size) if batch.episode_over[i] is False]).to(device=self.device)
        ...
        if self.params.get("dqn_type") == "DQN" or self.params.get("dqn_type") == "DuelingDQN":
            ...
            next_state_values = self.next_state_values_double_DQN(batch_size=batch_size, non_final_mask=non_final_mask,
                                                                  non_final_next_states=non_final_next_states)
        else:
    ...
    def next_state_values_DQN(self, batch_size, non_final_mask, non_final_next_states):
        ...
        if non_final_next_states.size()[
            0] > 0:  # All current states in this batch are the terminal states of their corresonpding sessions.
    ...
    def next_state_values_double_DQN(self, batch_size, non_final_mask, non_final_next_states):
        ...
        if non_final_next_states.size()[0] > 0:
            ...
            next_state_values[non_final_mask == 0] = self.target_net(non_final_next_states).gather(1,
                                                                                                   next_action_batch_current).detach().view(
                -1)
        return next_state_values
...
class DQN2(object):
    ...
    def save_model(self, model_performance, episodes_index, checkpoint_path):
        ...
        model_file_name = os.path.join(checkpoint_path,
                                       "model_d" + "_agent" + "_dqn" + "_s" + str(success_rate) + "_r" + str(
