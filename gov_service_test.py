...
from utils.word_match import is_multi_round, lev, longestCommonSubsequence, sigmoid
...
async def main_logic(para, mod, link, similarity_dict):
    global first_utterance, service_name, conv_id, end_flag, start_time, end_time, pipes_dict, positive_list
    ...
    async for websocket in websockets.connect(address, ping_interval=12000):
        try:
            ...
            if 'content' in msg.keys():
                user_msg = msg['content']['text'] if 'text' in msg['content'].keys() else msg['content']['service_name']
                log.info("user message:" + user_msg + str(conv_id))
            try:
                if msg['content']['service_name'] is not None:
                    ...
                    if service_name != '以上都不是' and pipes_dict[conv_id][9] != 0:
                        ...
                        recommend = get_recommend(service_name=pipes_dict[conv_id][7],
                                                  utterance=pipes_dict[conv_id][2], history=pipes_dict[conv_id][10])
            ...
            try:
                ...
                # Handle multiple rounds of dialogues  Continue to speak
                    ...
                    if 'content' not in msg.keys():
                        if isinstance(pipes_dict[conv_id][8], str):
                            messageSender(conv_id=conv_id, msg=pipes_dict[conv_id][8], log=log)
                        else:
                            messageSender(conv_id=conv_id, msg="大家都在问", options=pipes_dict[conv_id][8], log=log)
                        continue
                    multi = True
                    ...
                    if multi:
                        ...
                        messageSender(conv_id=conv_id, msg=answer, log=log, end=pipes_dict[conv_id][4])
                        recommend = get_recommend(service_name=pipes_dict[conv_id][7], utterance=pipes_dict[conv_id][2])
                        pipes_dict[conv_id][8] = recommend
                        pipes_dict[conv_id][2] = ""
                        messageSender(conv_id=conv_id, msg="大家都在问", log=log, end=True,
                                      service_name=service_name, options=recommend)
                        ...
                        p = Process(target=simulation_epoch,
                                    args=(
                                        (user_pipe[1], response_pipe[0]), agent, para, mod, log, similarity_dict,
                                        conv_id))
                        p.start()
                        # send_pipe, receive_pipe, first_utterance, process, single_finish, all_finish, first_utt,
